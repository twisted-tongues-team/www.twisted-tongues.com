...
To create a passage, go to the Passages tab, and click on “Add Passage”. Name the passage and describe it. If desired, passage metadata can be stored in the “Description”. It is recommended upon creating a passage to choose a previously defined *template* of tracks (see section 3), which will be present in each sentence in that passage. To choose a template, select one from the drop-down menu labelled “Tracks from template” (Figure 11). Alternatively, the user can select “None” in the drop-down menu and can customize the tracks added for the specific passage. Upon clicking “Submit”, the passage will be created and added to the list of passages in the Passages tab (Figure 12). Each passage's row ends in a menu button (⋮). “Rename…” there changes the passage's name and description, and “Delete…” removes the passage, after asking you to confirm that you would like to permanently delete it (Figure 13). Readers see no menu.
...
Next to each sentence that is a match for a given search, there is a “Context” button (cf. Figure 18). Clicking this button opens the sentence's passage scrolled to that sentence, which is highlighted, so a user can view the larger context in which that sentence was spoken. The address of that page does not change, so a user may copy it to share the sentence with a colleague who has access to the project, or keep it in a list of examples. Such links will continue to open the same sentence in future versions of TT.
...
Each project is only accessible by its owner and individuals who have been given access by the owner. The owner can grant access to a project from the “Share…” menu at the top right of the menu bar within the relevant project (Figure 24). Choose “Add Writer” or “Add Reader” according to the access you want to give, and enter the email address of the person being granted access (Figure 25). Click “Submit”. “View Shares” in the same menu lists everyone the project is shared with; a person marked “invited” has not yet logged in to TT with that address, and the button beside each one removes their access.
