...
Projects are sets of passages associated with a single research project. It is recommended that users create a different project for each distinct language for which data is entered into TT. Projects can be viewed from the “Projects” tab (Figure 2). Within the Projects tab, users will see every project they have created, together with those that have been shared with them, under two headings: “Shared projects” for those that have collaborators — whether you invited somebody to a project of yours, or somebody invited you to theirs — and “Private projects” for those that have none. A heading appears only when there is something under it, so a user who has never shared anything sees a single list. Each row also names your role in that project — owner, writer or reader — and the address of the person who owns it.

Clicking anywhere on a project's row opens it, giving access to the Dictionary, Word Search and Passages for that project, and to data entry. The name in the row is an ordinary link, so it can also be opened in a new tab. Everything else that can be done to a project sits behind the menu button (⋮) at the end of its row: “Rename…” changes the project's name and description; “Save to computer…” downloads the project as a file (see section 2.3); and “Delete…” permanently removes the project for everyone who can see it, after asking you to type “delete” to confirm. Rename and Delete are for the project's owner only; everybody else still sees them in the menu, greyed out and marked “owners only”. Save to computer is available to anyone who can open the project.

Figure 2\. Projects tab, with a project's menu open.
...
Any project can be saved to your own computer as a single file. Open the project and click “Save to Computer…” at the top right of the menu bar, or, without opening it, choose “Save to computer…” from the project's menu in the Projects tab. The browser will download one file named after the project (for example, “Florble fieldwork.json”) holding a complete snapshot of the project at that moment — passages, sentences and templates. Anyone who can open a project can save it, readers as well as writers.
...
To create a passage, go to the Passages tab, and click on “Add Passage”. Name the passage and describe it. If desired, passage metadata can be stored in the “Description”. It is recommended upon creating a passage to choose a previously defined *template* of tracks (see section 3), which will be present in each sentence in that passage. To choose a template, select one from the drop-down menu labelled “Tracks from template” (Figure 11). Alternatively, the user can select “None” in the drop-down menu and can customize the tracks added for the specific passage. Upon clicking “Submit”, the passage will be created and added to the list of passages in the Passages tab (Figure 12). Each passage's row ends in a menu button (⋮), as in the Projects tab. “Rename…” there changes the passage's name and description, and “Delete…” removes the passage, after asking you to confirm that you would like to permanently delete it (Figure 13). Readers see no menu.

Figure 11\. Add passage and select template.  
![](images/figure_011_add_passage_and_select_template.png)

Figure 12\. List of passages, with a passage's menu open.  
...
Next to each sentence that is a match for a given search, there is a “Context” button (cf. Figure 18). Clicking this button opens the sentence's passage scrolled to that sentence, which is highlighted, so a user can view the larger context in which that sentence was spoken.
...
Each project is only accessible by its owner and individuals who have been given access by the owner. The owner can grant access to a project from the “Share…” menu at the top right of the menu bar within the relevant project (Figure 24). Choose “Add Writer” or “Add Reader” according to the access you want to give, and enter the email address of the person being granted access (Figure 25); several addresses can be entered at once, separated by commas. Click “Submit”. Each address receives an email invitation with a link to the project, and that person can then log in with their Google account and open it. An address that has never logged in to TT is kept as an invitation, marked “invited” in the list of shares, and becomes a member the first time that person logs in. “View Shares” in the same menu lists everyone the project is shared with, writers and readers, and the button beside each one removes their access or withdraws the invitation.
