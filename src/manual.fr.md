...
Les projets sont des ensembles de passages associés à un même projet de recherche. Il est recommandé de créer un projet distinct pour chaque langue dont les données sont saisies dans TT. Les projets peuvent être consultés depuis l'onglet « Projects » (figure 2). Dans cet onglet, l'utilisateur voit tous les projets qu'il a créés, ainsi que ceux qui ont été partagés avec lui, répartis sous deux intitulés : « Shared projects » (projets partagés) pour ceux qui ont des collaborateurs — que vous ayez invité quelqu'un sur l'un de vos projets ou que l'on vous ait invité sur le sien — et « Private projects » (projets privés) pour ceux qui n'en ont aucun. Un intitulé n'apparaît que s'il a quelque chose à présenter : un utilisateur qui n'a jamais rien partagé ne voit donc qu'une seule liste. Chaque ligne indique également votre rôle dans le projet — « owner » (propriétaire), « writer » (rédacteur) ou « reader » (lecteur) — ainsi que l'adresse de son propriétaire.

Un clic n'importe où sur la ligne d'un projet l'ouvre et donne accès au dictionnaire (« Dictionary »), à la recherche de mots (« Word Search ») et aux passages (« Passages ») de ce projet, ainsi qu'à la saisie de données. Le nom figurant sur la ligne est un lien ordinaire : il peut donc aussi être ouvert dans un nouvel onglet. Tout le reste de ce que l'on peut faire d'un projet se trouve derrière le bouton de menu (⋮) en bout de ligne : « Rename… » (renommer) modifie le nom et la description du projet ; « Save to computer… » (enregistrer sur l'ordinateur) télécharge le projet sous forme de fichier (voir section 2.3) ; et « Delete… » (supprimer) supprime définitivement le projet pour tous ceux qui le voient, après vous avoir demandé de taper « delete » pour confirmer. « Rename » et « Delete » sont réservés au propriétaire du projet ; les autres les voient tout de même dans le menu, grisés et marqués « owners only » (propriétaires seulement). « Save to computer » est accessible à quiconque peut ouvrir le projet.

Figure 2\. Onglet « Projects », avec le menu d'un projet ouvert.
...
Chaque projet n'est accessible qu'à son propriétaire et aux personnes auxquelles celui-ci a donné accès. Le propriétaire peut accorder l'accès à un projet depuis le menu « Share… » (partager), en haut à droite de la barre de menu, dans le projet concerné (figure 24). Choisissez « Add Writer » (ajouter un rédacteur) ou « Add Reader » (ajouter un lecteur) selon l'accès que vous souhaitez donner, puis saisissez l'adresse électronique de la personne à qui l'accès est accordé (figure 25) ; plusieurs adresses peuvent être saisies à la fois, séparées par des virgules. Cliquez sur « Submit ». Chaque adresse reçoit par courriel une invitation contenant un lien vers le projet, et cette personne peut alors se connecter avec son compte Google et l'ouvrir. Une adresse qui ne s'est jamais connectée à TT est conservée comme invitation, marquée « invited » (invité) dans la liste des partages, et devient membre dès la première connexion de cette personne. « View Shares » (voir les partages), dans le même menu, liste toutes les personnes avec qui le projet est partagé, rédacteurs et lecteurs, et le bouton à côté de chacune retire son accès ou annule l'invitation.
